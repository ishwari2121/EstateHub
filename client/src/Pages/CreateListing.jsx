        ...
        if(files.length >0 && files.length + formData.imageUrls.length < 7)
            {
               ...
               Promise.all(promises).then((urls)=> {
                setFormData({
                    ...
                    setImageUploadError('Image uploadd failed (2 mb max per image)')
                    setUploading(false);
                })
        ...
        <form className='flex flex-col sm:flex-row gap-4'>
            <div className='flex flex-col gap-4 flex-1'>
                ...
                <div className='flex flex-wrap gap-6'>
                    <div className='flex items-center gap-2'>
                        ...
                        max='10' required className='p-3 border border-gray-300 rounded-lg'/>
                        <p>Beds</p>
                    </div>
                    <div className='flex items-center gap-2'>
                        <input type="number" id='bathroom' min='1' 
                        max='10' required className='p-3 border border-gray-300 rounded-lg'/>
                        <p>Baths</p>
                    </div>
                    <div className='flex items-center gap-2'>
                        ...
                        <div className='flex flex-col items-center'>
                            <p>Regular Price</p>
                            <span>(rupee/month)</span>
                        </div>
                    </div>
                    <div className='flex items-center gap-2'>
                        <input type="number" id='discountedPrice' min='1' 
                        max='10' required className='p-3 border border-gray-300 rounded-lg'/>
                        <div className='flex flex-col items-center'>
            ...
            formData.imageUrls.map((url, index) => (
              ...
              >
                ...
                disabled:opacity-80'>Create Listing</button>
            </div>
        </form>
    </main>
  )
